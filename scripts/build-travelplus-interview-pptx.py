...
def build():
    ...
    write_block(
        ...
        *L,
        [
            card_header("Содержание v1.14"),
            *labeled("Этапы 0–2", "Разведка, правила, границы продукта", gap=8),
            *labeled("Этапы 3–6", "Сценарий «как вести дело»", gap=8),
            *labeled("Инструменты", "StoryBrand, чеклист, реестр рисков, пилот", gap=8),
            *labeled(
                "Портфель",
                "Мостик Travel+: Hotel Line, Fleur, Natural, СТМ",
                gap=8,
            ),
            footnote("Образец для поля и собеседования. Не коммерческое предложение."),
        ],
    )
    write_block(
        s,
        *R,
        [
            ...
            *labeled("Глоссарии", "Бренд и ВЭД", gap=8),
            *labeled("Раздел L", "Путь товара, Incoterms, RFQ, приёмка", gap=8),
            *labeled("Волна A", "Markdown-реестры в папке brand-pult/", gap=8),
            *labeled("Синхрон", "Презентация и скрипт с пультом", gap=8),
            *labeled("Версия 2.0", "Когда технолог закроет цифры завода", gap=0),
        ],
    )
    ...
    write_block(
        ...
        *L,
        [
            card_header("Собрал"),
            *labeled("ДНК", "Обещание и границы линейки", gap=8),
            *labeled("StoryBrand", "Скрипт для продаж", gap=8),
            *labeled("Чеклист", "Готовность продукта перед рынком", gap=8),
            *labeled("Логистика", "Раздел L — путь при поставке из Китая", gap=8),
            footnote("Не прайс завода. Не коммерческое предложение."),
        ],
    )
    write_block(
        s,
        *R,
        [
            ...
            *labeled("Блокеры", "Приоритет этапа 2 с технологом", gap=8),
            *labeled("Сегмент", "Пилот eco / wellness", gap=8),
            *labeled(
                "Продажи",
                "Как не увести Natural в эконом Hotel Line",
                gap=8,
            ),
            *labeled("Вопросы", "Спасибо", gap=0),
        ],
    )
