...
def build():
    ...
    # 11 Cost expertise — plain copy + markup to buyer price
    s = prs.slides.add_slide(blank)
    add_bg(s)
    add_title(s, "Себестоимость: простыми словами")
    add_footer(s, 11)
    L, R = two_cards(s)
    write_block(
        s,
        *L,
        [
            card_header("До склада (ориентировочно)"),
            *labeled("Цена завода", "8 ₽ — у производителя до отправки", gap=8),
            *labeled("Доставка", "3 ₽ — море / самолёт, экспедитор, довоз", gap=8),
            *labeled("Таможня", "2 ₽ — пошлина, налог, брокер", gap=8),
            *labeled(
                "Запас",
                "1 ₽ — резерв на брак, пересорт и курс; это не прибыль",
                gap=8,
            ),
            *labeled("На складе", "14 ₽ — себестоимость единицы в Киришах", gap=8),
            footnote("Прайс для отеля считаю только от этой цифры."),
        ],
    )
    write_block(
        s,
        *R,
        [
            card_header("До цены закупщику (ориентировочно)"),
            *labeled("На складе", "14 ₽ за флакон", gap=8),
            *labeled(
                "Рынок",
                "Hotel Line 12–15 ₽ · ЕТС ~18 ₽ · Natura Siberica 30–40 ₽",
                gap=8,
            ),
            *labeled(
                "Наценка",
                "Смотрю маржу Travel+ и коридор «между Hotel Line и Natura Siberica»",
                gap=8,
            ),
            *labeled(
                "Цена закупщику",
                "Ориентир ~22–26 ₽ — выше Hotel Line, рядом с ЕТС, ниже Natura Siberica",
                gap=8,
            ),
            *labeled(
                "Проверка",
                "Если наценка не держится — меняю завод или не выхожу в прайс",
                gap=0,
            ),
        ],
    )
