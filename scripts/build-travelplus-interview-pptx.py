...
def build():
    ...
    # 11 Cost: stages + markup % + buyer price
    s = prs.slides.add_slide(blank)
    add_bg(s)
    ...
    write_block(
        ...
        *L,
        [
            ...
            *labeled("1. Цена завода", "8 ₽ — у производителя до отправки (FOB)", gap=8),
            *labeled("2. Доставка", "3 ₽ — море / самолёт, экспедитор, довоз", gap=8),
            *labeled("3. Таможня", "2 ₽ — пошлина, налог, брокер", gap=8),
            *labeled(
                "4. Запас",
                "1 ₽ — резерв на брак, пересорт и курс; не прибыль",
                gap=8,
            ),
            *labeled("5. На складе", "8 + 3 + 2 + 1 = 14 ₽ за флакон", gap=8),
            *labeled(
                "FOB",
                "Поставщиков сравниваю по цене FOB — одинаковая база, без доставки",
                gap=0,
            ),
        ],
    )
    write_block(
        ...
        *R,
        [
            card_header("До закупщика (ориентировочно)"),
            *labeled("Себестоимость", "14 ₽ на складе в Киришах", gap=8),
            *labeled(
                "Рынок",
            ...
            *labeled(
                "Наценка",
                "~70% к себестоимости — маржа Travel+ в коридоре рынка",
                gap=8,
            ),
            *labeled(
                "Цена закупщику",
                "14 ₽ × 1,7 ≈ 24 ₽ — выше Hotel Line, рядом с ЕТС, ниже Natura Siberica",
                gap=8,
            ),
            *labeled(
                "Проверка",
                "Наценка не держится — меняю завод или не выхожу в прайс",
                gap=0,
            ),
        ],
    )
