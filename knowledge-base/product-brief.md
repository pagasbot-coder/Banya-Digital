...
**ICP (Human Architect, 2026-05-25):** владелец или управляющий **одного премиум-банного комплекса** (несколько зон/залов, F&B под ритуал, kitchen↔SPA) — **не** primary ICP: urban-термы (отдельный демо-пресет возможен позже, T-020; позиционирование и пилот — **сначала премиум-баня**). Запись/касса уже есть (YCLIENTS, 1С, Excel), но нет **пульта смены с данными по каждой зоне**, yield и прозрачного COGS. Banya-Digital — **операционный B2B-дашборд**, не замена онлайн-записи гостя. **Командный ревью:** `@knowledge-base/SPA-SEGMENT-TEAM-REVIEW.md` (часть C). Детали: `@knowledge-base/segment-spa-banya-analysis.md`, `@knowledge-base/industry-brief.md`.
...
**WAMZ** (*Weekly Active Managed Zones*) — **количество залов** (`Hall`), по которым за **скользящие 7 календарных дней** (таймзона пилота: **Europe/Moscow**) зафиксирована **хотя бы одна** операционная активность.

**Зона считается активной за неделю**, если для её `hallId` выполнено **любое** из:

1. **Финансы:** есть `RevenueLine` **или** `CostLine` с `businessDate` в окне 7 дней.
2. **Операции:** есть `ShiftChecklist` с `shiftDate` в окне и **≥1** отмеченный `ChecklistItem` (`completedAt` не null).
3. **CRM / загрузка:** есть `Booking` по залу с изменением статуса или времени (`updatedAt`) в окне (обновление брони, не только создание seed).

**Знаменатель:** все залы tenant (в демо/пилоте seed — **4** зала: Парная, VIP, Сеновал, Хамам; см. `prisma/seed.ts`).

| Параметр | Значение |
|----------|----------|
| **Цель пилота (неделя 8)** | **WAMZ ≥ 3 из 4** (устойчивый ритм по зонам) |
| **Stretch** | **WAMZ = 4/4** две недели подряд к концу пилота |
| **Co-metrics** (не North Star) | −30% время закрытия смены; ≥95% остатки FIFO; daily margin на dashboard — см. таблицу ниже |

**Смысл:** North Star измеряет **привычку вести объект по зонам в системе**, а не разовый просмотр сводки владельцем.

**Dashboard (backlog, не P0 пилота):** карточка «WAMZ **X/4**» + тренд к прошлой неделе; drill-down — какие зоны «спят» (нет revenue/checklist/booking). Задача для PM → Developer после стабилизации T-011…013 (кандидат post-pilot KPI).
...
|---------|----------|---------------------------|--------------|
| **WAMZ (North Star)** | 0–1 зоны в Excel | **≥3/4** к нед. 8; stretch **4/4** | Подсчёт по правилам выше; PM — еженедельно |
...
| Видимость маржи по залу | Ручная сводка 1×/нед | **Daily** на dashboard | KPI при `RevenueLine`/`CostLine` за день (co-metric) |
